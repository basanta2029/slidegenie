...
# router.add_exception_handler(RateLimitExceeded, _rate_limit_exceeded_handler)  # APIRouter doesn't have add_exception_handler
...
async def get_storage_manager() -> Optional[StorageManager]:
...
async def get_async_processor() -> Optional[AsyncDocumentProcessor]:
    """Get async document processor instance."""
    # Check if Supabase credentials are configured
    if not settings.SUPABASE_URL or not settings.SUPABASE_SERVICE_KEY:
        logger.warning("Supabase storage credentials not configured for async processor")
        return None
...
async def initiate_upload(
    ...
    async_processor: Optional[AsyncDocumentProcessor] = Depends(get_async_processor)
):
    """
    ...
    try:
        ...
        if not storage_manager or not async_processor:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="Storage service is not available. Please contact support."
            )
