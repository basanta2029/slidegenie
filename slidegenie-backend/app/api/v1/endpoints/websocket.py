...
class WebSocketManager:
    ...
    async def initialize_dependencies(self):
        ...
        logger.info("Initializing WebSocket manager dependencies")
        
        # Initialize progress tracker (required)
        try:
            ...
            # Progress tracker is required, so we raise
            raise
        ...
        try:
            ...
            if settings.SUPABASE_URL and settings.SUPABASE_SERVICE_KEY:
                ...
                logger.info("Async document processor initialized")
            else:
                logger.warning("Supabase storage credentials not configured, async processor disabled")
                self.async_processor = None
        except Exception as e:
            logger.warning(f"Failed to initialize async processor: {e}")
            logger.warning("WebSocket will run with limited functionality")
            self.async_processor = None
        ...
        try:
            ...
            if settings.SUPABASE_URL and settings.SUPABASE_SERVICE_KEY:
                ...
                logger.warning("Supabase storage credentials not configured, storage manager disabled")
        ...
        logger.info("WebSocket manager dependencies initialization complete")
        ...
        while True:
            try:
                ...
                if message_type == "subscribe":
                    ...
                    if job_id:
                        if websocket_manager.async_processor:
                            try:
                            ...
                            await connection.send_error(
                                "Job processing service is not available",
                                "SERVICE_UNAVAILABLE"
                            )
                    else:
                        await connection.send_error(
