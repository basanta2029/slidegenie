...
class AsyncDocumentProcessor:
    ...
    async def initialize(self) -> None:
        ...
        try:
            # Initialize storage manager (optional - fail gracefully)
            try:
                await self.storage_manager.initialize()
                logger.info("Storage manager initialized")
            except Exception as e:
                logger.warning(f"Storage manager initialization failed: {e}")
                logger.warning("Document processing will work with limited functionality")
                # Set storage_manager to None to indicate it's not available
                self.storage_manager = None
            
            # Initialize task queue (required)
