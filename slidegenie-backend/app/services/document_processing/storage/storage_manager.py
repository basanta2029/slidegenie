...
# Try to import real components, fall back to mocks for MVP
try:
    from .cache_manager import CacheManager
except Exception as e:
    logging.warning(f"Failed to import CacheManager, using mock: {e}")
    from .mock_components import MockCacheManager as CacheManager

try:
    from .search_indexer import SearchIndexer
except Exception as e:
    logging.warning(f"Failed to import SearchIndexer, using mock: {e}")
    from .mock_search_indexer import MockSearchIndexer as SearchIndexer

try:
    from .lifecycle_manager import LifecycleManager
except Exception as e:
    logging.warning(f"Failed to import LifecycleManager, using mock: {e}")
    from .mock_components import MockLifecycleManager as LifecycleManager

try:
    from .backup_manager import BackupManager
except Exception as e:
    logging.warning(f"Failed to import BackupManager, using mock: {e}")
    from .mock_components import MockBackupManager as BackupManager

from .supabase_storage import get_storage_client, SupabaseStorageClient
...
class StorageManager:
    ...
    async def _process_and_cache_content(
        ...
        processed_content = {
            "text_content": content.decode('utf-8', errors='ignore')[:5000],  # Extract first 5000 chars
    ...
    async def _retrieve_original_file(self, file_id: str) -> bytes:
        ...
        try:
            ...
            # Try to find the file by listing files with the file_id prefix
            files = storage_client.list_files(prefix=f"documents")
            
            # Find the file with matching file_id in the path
            target_file = None
            for file in files:
                if file_id in file['key']:
            ...
            # Download the file
            content = storage_client.download_file(target_file)
            logger.info(f"Retrieved file {file_id} from storage")
            return content
            
        except Exception as e:
            logger.error(f"Failed to retrieve original file {file_id}: {e}")
            raise
    ...
    async def _delete_original_file(self, file_id: str) -> None:
        ...
        try:
            ...
            # Find the file by listing files with the file_id prefix
            files = storage_client.list_files(prefix=f"documents")
            
            # Find and delete all files with matching file_id
            deleted_count = 0
            for file in files:
                if file_id in file['key']:
                    if storage_client.delete_file(file['key']):
                        deleted_count += 1
                        logger.info(f"Deleted file: {file['key']}")
            
            if deleted_count == 0:
                logger.warning(f"No files found for file_id {file_id}")
            else:
                logger.info(f"Deleted {deleted_count} file(s) for file_id {file_id}")
                
        except Exception as e:
            logger.error(f"Failed to delete original file {file_id}: {e}")
            raise
