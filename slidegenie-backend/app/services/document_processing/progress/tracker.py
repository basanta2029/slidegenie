...
class ProgressTracker:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Convert RedisDsn to string if necessary
            redis_url_str = str(self.redis_url) if self.redis_url else None
            ...
            if redis_url_str:
                try:
                    ...
                    logger.warning(f"Redis connection failed: {redis_error}. Running without Redis persistence.")
                    self.redis_pool = None
            else:
                logger.warning("No Redis URL configured. Running without Redis persistence.")
                self.redis_pool = None
            
            # Start WebSocket server (only if Redis is available for now)
            if self.redis_pool:
                ...
                logger.warning("WebSocket server disabled due to missing Redis connection")
    ...
    async def get_job_progress(self, job_id: UUID) -> Optional[ProcessingProgress]:
        ...
        try:
            ...
            # Check Redis for completed jobs (if available)
            if self.redis_pool:
    ...
    async def _persist_job_history(self, job_history: JobProgressHistory) -> None:
        ...
        if not self.redis_pool:
            # Skip persistence if Redis is not available
            return
