...
# Optional imports - these are only needed if GoogleSlidesGenerator is actually used
...
try:
    ...
    # Log warning but don't fail the entire application
    import warnings
    warnings.warn(
        f"Google Slides export functionality unavailable: {e}. "
        "To enable, install: pip install google-auth google-auth-oauthlib google-auth-httplib2 "
...
class GoogleSlidesGenerator:
    ...
    def __init__(
        ...
        if not GOOGLE_IMPORTS_AVAILABLE:
            raise RuntimeError(
                "Google Slides export functionality is not available. "
