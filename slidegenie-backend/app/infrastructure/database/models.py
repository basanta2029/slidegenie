...
class Presentation(Base, TimestampMixin):
    ...
    # Additional metadata
    presentation_metadata = Column("metadata", JSONB, default=dict)
...
class PresentationEmbedding(Base):
    ...
    embedding_vector = Column('embedding', pgvector.VECTOR(1536) if pgvector else JSONB)  # OpenAI ada-002 dimension
    ...
    # Indexes
    __table_args__ = (
        Index('idx_embedding_vector', 'embedding', postgresql_using='ivfflat') if pgvector else tuple(),
    )
...
# pgvector extension already imported at the top of the file